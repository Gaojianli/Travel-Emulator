...
void TravelEmulator::form::timeMagicCallback()
{
	...
	if (result->Count > currentShiftID+1 && result[currentShiftID + 1]->start < DateTime::Parse(timeLabel->Text)) {
		if (currentShiftID != 0)
			currentShiftID++;
	...
	else if (result->Count <= currentShiftID + 1) {
		timeMagic->Enabled = false;
